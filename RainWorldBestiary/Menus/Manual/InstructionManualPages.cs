...
namespace RainWorldBestiary.Menus.Manual
{
    ...
    internal class EntriesPage : InstructionManualPage
    {
        public EntriesPage(Menu.Menu menu, MenuObject owner)
            : base(menu, owner)
        {
            MenuIllustration menuIllustration = new MenuIllustration(menu, owner, MenuResources.IllustrationsEntryIconsPath, "Survivor_head", belowHeaderPosCentered - verticalBuffer, crispPixels: true, anchorCenter: true)
            {
                sprite =
                {
                    scale = 4f
    ...
    internal class UnlockingPageFirst : InstructionManualPage
    {
        ...
        private void AddPips(Menu.Menu menu, int count, Color color, bool includeTitlePip = false)
        {
            ...
            if (includeTitlePip)
            {
                MenuIllustration pip = new MenuIllustration(menu, this, MenuResources.IllustrationsIconsPath, unl ? MenuResources.UnlockPipUnlockedName : MenuResources.UnlockPipName, new Vector2(PX, PipY), true, true)
                {
                    sprite =
                    {
                        scale = 2f
        ...
        private void AddRandomPip(Menu.Menu menu, Color color, out bool unlocked)
        {
            ...
            MenuIllustration pip = new MenuIllustration(menu, this, MenuResources.IllustrationsIconsPath, unlocked ? MenuResources.UnlockPipUnlockedName : MenuResources.UnlockPipName, new Vector2(PipX, PipY), true, true)
            {
                color = color,
                sprite =
                    {
    ...
    internal class UnlockingPageSecond : InstructionManualPage
    {
            ...
            : base(menu, owner)
        {
            ...
            MenuIllustration pip = new MenuIllustration(menu, this, MenuResources.IllustrationsIconsPath, MenuResources.UnlockPipName, new Vector2(currentPipX, currentPipY - 10), true, true)
            {
                sprite = { scale = 3f }
            };
            subObjects.Add(pip);
            ...
            pip = new MenuIllustration(menu, this, MenuResources.IllustrationsIconsPath, MenuResources.UnlockPipUnlockedName, new Vector2(currentPipX, currentPipY - 10), true, true)
            {
                sprite = { scale = 3f }
            };
            subObjects.Add(pip);
            ...
            foreach ((DescriptionModule.PredefinedColors color, string title) color in predefinedColors)
            {
                ...
                pip = new MenuIllustration(menu, this, MenuResources.IllustrationsIconsPath, MenuResources.UnlockPipUnlockedName, new Vector2(currentPipX, currentPipY), true, true)
                {
                    sprite = { scale = 3f },
                    color = c
                };
